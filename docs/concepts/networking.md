...
Private ARO ingress does not make an application publicly reachable on its own. `front_door` provisions the edge service and connects it to the private ingress over Private Link, but it does not finish the job: you still own DNS for custom domains, the application and its routes, and the security policy you want on the WAF. A cluster with no matching route returns `503` through Front Door exactly as it does internally.
