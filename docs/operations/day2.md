...
`front_door` provisions the edge service, but it does not deliver a working application on its own. Operators still own the workload and its routes, DNS for custom domains, security policy on the WAF, and observability.
