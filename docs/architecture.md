...
The bootstrap and workload states are separate. Bootstrap starts local and creates the remote backend used by workload Terraform. The two GitHub environments use separate Entra applications. No client credential is issued; federated subjects are `repo:<owner>/<repo>:environment:plan|apply`.
